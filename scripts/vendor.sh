...
HG_RAW="https://hg.mozilla.org/mozilla-central/raw-file/tip"
...
declare -a items=(
  ...
  "$VENDOR/bergamot-translator.js | $HG_RAW/toolkit/components/translations/bergamot-translator/bergamot-translator.js | "
...
for item in "${items[@]}"; do
  ...
  if [[ -n "$expected" ]]; then
    ...
    echo "OK    $(basename "$dest") (no sha256 pin — source: mozilla-central tip)"
  fi
done
